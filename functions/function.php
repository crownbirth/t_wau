<?php
function migrate_details($row_ses, $jamb_no, $tams, $type = 'new') {
    $level = 1;
    $sesid = $row_ses['sesid'];
    ...
    $query_matric = sprintf("SELECT p.last, c.colcode, p.progcode FROM programme p "
    ...
    // Update last matric
    $query_update = sprintf("UPDATE programme SET last = last + 1 WHERE progid = %s", 
            GetSQLValueString($row_details['progofferd'], "int"));
    $update =  mysql_query($query_update, $tams) or die(mysql_error());
    
    $gen_matric = $ses_year.$row_matric['colcode'].$row_matric['progcode'].str_pad($row_matric['last'], 3, '0', STR_PAD_LEFT);
    ...
    if($type = 'new') {
        ...
        $query = sprintf("INSERT INTO student (stdid, lname, fname, mname, progid, phone, email, addr, sex, dob, sesid, level, admode, password, stid, jambno) VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
        ...
        $query = sprintf("Update student SET stdid = %s WHERE jambno = %s",
                                GetSQLValueString($gen_matric, "text"),
                                GetSQLValueString($jamb_no, "text"));
    }
...
function doLogin($user, $loginUsername, $password, $db, $external = FALSE) {
	...
	if( $user == 1 ){
		$LoginRS__query=sprintf("SELECT formnum, pstdid, fname, lname, jambregid, access, formsubmit, formpayment 
                                        FROM prospective 
                                        WHERE jambregid=%s 
                                        AND lname=%s",
