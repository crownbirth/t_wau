...
mysql_select_db($database_tams, $tams);
$query_cat = "SELECT ct.catid, ct.catname FROM category ct WHERE ct.type=1 ORDER BY ct.catname";
...
if (isset($row_cat['catid'])) {
  $colname_cat = $row_cat['catid'];
}

if (isset($_GET['catid'])) {
  $colname_cat = $_GET['catid'];
}
...
mysql_select_db($database_tams, $tams);
$query_deptcrs = sprintf("SELECT DISTINCT c.csid, csname, upload, approve FROM course c, department_course dc, teaching t WHERE c.csid = dc.csid AND c.csid = t.csid AND dc.csid = t.csid AND c.type='General' AND t.sesid = %s AND c.catid=%s ORDER BY csid ASC", 
						GetSQLValueString($colname1_deptcrs, "int"), 
						GetSQLValueString($colname_cat, "int"));
...
<?php include '../include/topmenu.php'; ?>
  ...
  <div class="pagetitle">
    <table width="600">
      <tr>
        <td><!-- InstanceBeginEditable name="pagetitle" -->Consider Result for General Courses<!-- InstanceEndEditable --></td>
      </tr>
    </table>
  </div>
<div class="sidebar1">
  ...
  <div class="content"><!-- InstanceBeginEditable name="maincontent" -->
    <table width="690">		
      ...
      <tr>
        <td width="279">
         Category
          <select name="catid" id="catid" onchange="catfilt(this)">
            <?php
			do {  
			?>
            <option value="<?php echo $row_cat['catid']?>"<?php if (!(strcmp($row_cat['catid'], $colname_cat))) {echo "selected=\"selected\"";} ?>><?php echo $row_cat['catname']?></option>
            <?php
            ...
            <table width="683" border="0">
			  ...
			  <?php do{?>
                <tr>
                  ...
                  <td width="385"><a href="result.php?csid=<?php echo $row_deptcrs['csid']?>&sid=<?php echo  $colname1_deptcrs?>"><?php echo ucwords(strtolower($row_deptcrs['csname']))?></a></td>
