...
//    var_dump($_POST);
    $updateSQL = sprintf("UPDATE result_buffer SET cleared=%s "
                            . "WHERE stdid=%s "
                            . "AND sesid=%s",
...
$query_cour = sprintf("SELECT r.cleared, c.csid, c.csname, c.semester, d.unit, d.status "
                        . "FROM result_buffer r "
                        . "JOIN course c ON r.csid = c.csid "
                        . "JOIN department_course d ON d.csid = r.csid AND c.csid = d.csid "
                        . "WHERE r.sesid = %s " 
                        . "AND d.progid = %s "
                        . "AND stdid = %s",                        
                        GetSQLValueString($row_rssess['sesid'], "int"),
                        GetSQLValueString($row_studs['progid'], "int"),
                        GetSQLValueString($colname_stud, "text"));
$cour = mysql_query($query_cour, $tams) or die(mysql_error());
$row_cour = mysql_fetch_assoc($cour);
$totalRows_cour = mysql_num_rows($cour);

$query_pcour = sprintf("SELECT r.cleared, c.csid, c.csname, c.semester, d.unit, d.status "
                        . "FROM result_buffer r "
                        . "JOIN course c ON r.csid = c.csid "
                        . "JOIN department_course d ON d.csid = r.csid AND c.csid = d.csid "
                        . "WHERE d.progid = %s " 
                        . "AND r.sesid = %s "
                        . "AND stdid = %s",
                        GetSQLValueString($row_pstuds['progid'], "int"),
                        GetSQLValueString($row_rssess['sesid'], "int"),
...
<div class="container">
  ...
  <div class="pagetitle">
    <table width="600">
      <tr>
        <td><!-- InstanceBeginEditable name="pagetitle" -->Course Form <?php //echo $row_stud['lname'].", ".$row_stud['fname']." (".$colname_stud.")";?><!-- InstanceEndEditable --></td>
      </tr>
    </table>
  </div>
<div class="sidebar1">
  ...
  <div class="content"><!-- InstanceBeginEditable name="maincontent" -->
    <table width="679" border="0" class="mytext">
                                ...
                                <?php if($totalRows_studs){?>
                                    <table width="660">
                                            ...
                                            <td colspan="5" align="right">
                                              <select onChange="studfilt(this)" name="stdid">
                                                <?php
                                                    do {  
                                                ?>
                                                <option value="<?php echo $row_studs['stdid']?>" <?php if($colname_stud == $row_studs['stdid']) echo 'selected'?>>
                                                    <?php echo ucwords(strtolower($row_studs['fname']." ".$row_studs['lname']))."(".$row_studs['stdid'].")"?>
                                                </option>
                                                <?php
                                        ...
                                        <tr>
                                            ...
                                            <td><input type="checkbox" name="course[]" value="<?php echo $row_cour['csid']?>" <?php if($row_cour['cleared'] == 'TRUE')echo 'checked'?>/></td>
                                        </tr>
                                    ...
                                    <?php if($totalRows_pstuds){?>
                                      <table width="660">
                                              ...
                                              <td colspan="5" align="right">
                                                <select onChange="studfilt(this)" name="stdid">
                                                  <?php
                                                      do {  
                                                  ?>
                                                  <option value="<?php echo $row_pstuds['stdid']?>" <?php if($colname_stud == $row_pstuds['stdid']) echo 'selected'?>>
                                                      <?php echo ucwords(strtolower($row_pstuds['fname']." ".$row_pstuds['lname']))."(".$row_pstuds['stdid'].")"?>
                                                  </option>
                                                  <?php
                                          ...
                                          <tr>
                                              ...
                                              <td><input type="checkbox" value="<?php echo $row_pcour['csid']?>" <?php if($row_pcour['cleared'] == 'TRUE')echo 'checked'?>/></td>
                                          </tr>
      ...
      <script>
          var TabbedPanel1 = new Spry.Widget.TabbedPanels("TabbedPanels1");
      </script>                   
