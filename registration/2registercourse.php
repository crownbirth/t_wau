...
mysql_select_db($database_tams, $tams);
$query_course = sprintf("SELECT r.csid, c.semester, c.csname, d.status, d.unit FROM result r, course c, department_course d WHERE r.cleared = true AND r.stdid = %s AND c.csid = r.csid AND d.csid = r.csid AND r.sesid = %s AND d.progid=%s ORDER BY r.csid, c.semester ASC", 
                            GetSQLValueString($colname_regStatus, "text"), 
