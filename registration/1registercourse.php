...
mysql_select_db($database_tams, $tams);
$query_stud = sprintf("SELECT s.stdid, s.disciplinary, s.fname, s.payment, s.lname, s.level, s.progid, p.progname, d.deptname FROM student s, programme p, department d WHERE s.progid = p.progid AND p.deptid = d.deptid AND stdid = %s", GetSQLValueString($colname_stud, "text"));
...
mysql_select_db($database_tams, $tams);
$query_course = sprintf("SELECT r.csid, c.semester, c.csname, d.status, d.unit FROM result_buffer r, course c, department_course d WHERE r.cleared = true AND r.stdid = %s AND c.csid = r.csid AND d.csid = r.csid AND r.sesid = %s AND d.progid=%s ORDER BY r.csid, c.semester ASC", 
                            GetSQLValueString($colname_regStatus, "text"), 
...
$crsAppr = false;
if($row_regStatus['approved'] == "TRUE" )
	$crsAppr = true;
...
$query_ref = sprintf("SELECT d.csid, status, csname, unit FROM department_course d JOIN course c ON c.csid = d.csid WHERE progid = %s AND %s",
                        GetSQLValueString($row_stud['progid'], "int"),
...
mysql_select_db($database_tams, $tams);
$query_cur = sprintf("SELECT d.csid, status, csname, unit FROM department_course d JOIN course c ON c.csid = d.csid WHERE progid = %s AND d.csid LIKE %s",
...
<div class="sidebar1">
  ...
  <div class="content"><!-- InstanceBeginEditable name="maincontent" -->
                ...
                <table width="644">
                  <tr>
                    <td colspan="3" valign="top" style="color:red">
                    	Please check your Department Handbook for courses to register. Courses with course code ending with alphabet 'n' are mainly for 100 - 200 level students. 
                    </td>
                  <tr>
                  <tr>
                  ...
                  <tr>
                    <td colspan="3" valign="top">                        
                    	<fieldset id="ref-field">
                            <button data-toggle="modal" href="#modal-2" class="btn btn-primary btn-medium">Add References</button>
                    ...
                    <td colspan="3" valign="top">
                    	<fieldset>
                            ...
                            <div>
                                ...
                                <p style="float:left;"><?php echo $row_cur['csname']?> (<span><?php echo $row_cur['unit']?></span> Units, <?php echo $row_cur['status']?>)</p>
      ...
      <tr>
      	<td>
       	  <table border="0" align="center">
              <tr>
                <td colspan="2" align="right"><a href="courseform.php<?php echo "?sid=".$colname_regsess;?>" target="_new">Print Form</a>
    ...
    <?php     
                ...
                echo "You are required to pay for the session before registration! <a href='http://portal.tasued.edu.ng/regular_students/schfee/main/status.php' target='_Blank'>Click here</a> to proceed to payment.";
            }

          }else {
              echo "You are on a disciplinary action<strong>  {$row_rsdisp['status']}</strong> as at <strong> {$row_rsdisp['login']} </strong> Kindlly  contact the Registrar's Office for advice and necessary action" ;
          }
      }else {
    ...
    <script type="text/javascript">
        $(function() {
            $('#add-courses').click(function() { 
                ...
                var rem = $('#rem');
		var reg = $('#reg');
                var checked = $('#modal-2 input[type=checkbox]:checked').parent().parent();
                $('#ref-field input:checked').parent().parent().find('span').each(function() {
                    totalUnits += parseInt($(this).text());
                });
                reg.text( parseInt(reg.text()) - totalUnits);
		rem.text( parseInt(rem.text()) + totalUnits);
                $('#ref-field').children('div').remove();
                $('#ref-field').append(checked);
                $('#modal-2').modal('hide');
            });
        });
    </script>
