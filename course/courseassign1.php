...
mysql_select_db($database_tams, $tams);
$query_courses = sprintf("SELECT c.csid, c.csname, d.colid, ct.catname FROM course c, category ct, department d WHERE d.deptid = c.deptid AND ct.catid = c.catid AND (c.type = 'General' OR c.type = 'College' AND d.colid = %s OR c.deptid = %s) ORDER BY c.csid",
...
mysql_select_db($database_tams, $tams);
$query_prog = sprintf("SELECT * FROM programme WHERE deptid = %s", 
