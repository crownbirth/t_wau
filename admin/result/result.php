...
//	*/
//		mysql_select_db($database_tams, $tams);
//		$query_rstemp = sprintf("SELECT * FROM result WHERE csid=%s AND sesid=%s",
//							 GetSQLValueString($csid, "text"), 
//							 GetSQLValueString($sesid, "int"));
//		$rstemp = mysql_query($query_rstemp, $tams) or die(mysql_error());
//		$row_rstemp = mysql_fetch_assoc($rstemp);
//		$totalRows_rstemp = mysql_num_rows($rstemp);
//		
//		mysql_query("BEGIN");
//		do{				
//			$insert_query = sprintf("UPDATE (stdid, csid, sesid, tscore, escore) VALUES(%s, %s, %s, %s, %s)",
//						   GetSQLValueString($row_rstemp['stdid'], "text"),
//						   GetSQLValueString($csid, "text"),
//						   GetSQLValueString($sesid, "int"),
//						   GetSQLValueString($row_rstemp['tscore'], "int"),
//						   GetSQLValueString($row_rstemp['escore'], "int"));
//											
...
//			  }
//			  else{		
//				  $insert_error[] = $row_rstemp['stdid'];
//				  $uploaded = false;
//			  }
//		}while ($row_rstemp = mysql_fetch_assoc($rstemp)) ;
//	  
//		if( $uploaded ){
...
//			$uploadstat = "Approval Unsuccessful! Result for the following student(s) could not be approved:<br/>";
//			foreach( $insert_error as $error){
//				$uploadstat .= $error."<br/>";
//			}
//			
//			$uploadstat .= "Please contact system admin.";
//		}
//			
            mysql_query("BEGIN");	
...
mysql_select_db($database_tams, $tams);
$query_status = sprintf("SELECT approve, progname,upload, type FROM course c, teaching t, programme p WHERE c.csid = t.csid AND t.deptid = p.deptid AND t.deptid = %s AND sesid = %s AND t.csid = %s", 
      ...
      <tr>
        <td colspan="2">
            ...
            <?php do{?>                
                ...
                <td class="matric" >
                	<a target="_blank" href="../../student/profile.php?stid=<?php echo $row_rslt['stdid']?>"><?php echo $row_rslt['stdid']?></a>
