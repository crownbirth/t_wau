...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
	$password = htmlentities($row_editprof['password']);
	if(isset($_POST['password'])){
		$password = md5($_POST['password']);
		}
	
  $updateSQL = sprintf("UPDATE lecturer SET title=%s, fname=%s, lname=%s, mname=%s, phone=%s, deptid=%s, email=%s, addr=%s, sex=%s, password=%s, profile=%s WHERE lectid=%s",
