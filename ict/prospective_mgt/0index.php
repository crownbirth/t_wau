...
$query_second = "SELECT progid1, status, count(pstdid) as count "
                    . "FROM prospective p "
...
for($idx = 0; $idx < $totalRows_second; $idx++, $row_second = mysql_fetch_assoc($second)) {
    if(isset($second_choice[$row_second['progid1']])) {
        $second_choice[$row_second['progid1']] = $row_second['count'];
    }
}

$query_second = "SELECT progid2, status, count(pstdid) as count "
                    . "FROM prospective p "
                    . "JOIN schfee_transactions s ON p.jambregid = s.can_no "
                    . "GROUP BY progid2 "
                    . "HAVING status = 'APPROVED'";
...
for($idx = 0; $idx < $totalRows_second; $idx++, $row_second = mysql_fetch_assoc($second)) {
    if(isset($second_choice[$row_second['progid2']])) {
