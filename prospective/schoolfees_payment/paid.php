...
//		$result = mysql_query($sqlget);
                        ...
                        $query_paid= sprintf("UPDATE prospective SET acceptance=%s WHERE jambregid=%s", GetSQLValueString("Yes", "text"), GetSQLValueString(getSessionValue('MM_Username'), "text"));
                        $paid= mysql_query($query_paid, $tams);
...
if($rsResult['email']!=null){
    ...
    $body ="Good day {$portalName},\nYou have successfully paid form your POST UTME/DE Acceptance fee\n==Payment Details==\n"
                . "Card PAN: {$pan}\nCard Holder: {$name}\nUnique ID: {$ordid}"
